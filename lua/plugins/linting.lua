  ...
  opts = function(_, opts)
    ...
    local compound_part_ft_linters = vim
      ...
      :map(function(ft, linters)
        ...
        local is_compound_ft = linters._compound_ft == nil and default_compound_ft or linters._compound_ft
        --- NOTE: 移除标志转为 string[]
      ...
      :fold({}, function(acc, ft, part_ft_linters)
    ...
    for ft, part_ft_linters in pairs(compound_part_ft_linters) do
      ...
      for part_ft, linters in pairs(part_ft_linters) do
        -- 合并去重
        ft_linters[part_ft] = vim.iter({ ft_linters[part_ft] or {}, linters }):flatten():unique():totable()
      end
    end
  end,
}
